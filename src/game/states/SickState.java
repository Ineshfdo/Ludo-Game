...
public class SickState extends EffectState {

    public SickState(PieceState underlyingState, int roundsRemaining) {
    ...
    public void decrementRoundsPreTurn(LudoPiece piece) {
        if (roundsRemaining > 0) {
            roundsRemaining--;
            if (roundsRemaining == 0) {
                System.out.println("  -> " + piece.getId() + "'s Sick effect has worn off.");
