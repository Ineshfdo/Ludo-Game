...
public class CaptureManager {
    ...
    public static boolean handleStandardCellLanding(LudoPiece piece, int newPos, boolean ignoreMysteryCell) {
        ...
        java.util.Map<game.players.components.PlayerColor, List<String>> capturedMap = new java.util.HashMap<>();
        ...
        if (captured) {
            ...
            boolean first = true;
            for (java.util.Map.Entry<game.players.components.PlayerColor, List<String>> entry : capturedMap
                    .entrySet()) {
                if (!first)
                    sb.append(" and ");
                String capName = entry.getKey().toString().substring(0, 1).toUpperCase()
